...
const TEMPLATES = {
  ...
  terracotta:    { name: "Terracotta",     accent: "#9c4221", bg: "#fdf2ee", text: "#2d1a14", secondary: "#888",    headerBg: "#9c4221", headerText: "#ffffff", sidebar: "#fbe8df" },
    ...
    if (mode === "login") {
      ...
      if (error) { setError(error.message); }
        ...
        await supabase.from("profiles").upsert({
          id: data.user.id, email, plan: "paypercv", cv_count: 0, selected_template: null
        });
        onAuth(data.user);
      }
    }
    ...
    <>
      ...
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f0c29, #1e1b4b, #302b63)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'DM Sans', sans-serif", padding: 20 }}>
        <div style={{ background: "rgba(255,255,255,0.05)", backdropFilter: "blur(20px)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 20, padding: "40px 36px", width: "100%", maxWidth: 420 }}>
          ...
          {error && (
            <div style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, padding: "10px 14px", color: "#fca5a5", fontSize: 13, marginBottom: 16 }}>
              {error}
            </div>
          )}
  ...
  const plans = [
    {
      ...
      features: [
        { label: "1 CV par paiement", ok: true },
        { label: "Tous les modèles", ok: true },
        { label: "Export PDF", ok: true },
        { label: "CV sauvegardé", ok: true },
        { label: "CV illimités", ok: false },
        { label: "Génération IA", ok: false },
      ],
      cta: payMode ? "💳 Payer 500F et télécharger" : "Payer 500F",
      active: false, highlight: false, montant: 500, planKey: "paypercv",
    },
    {
      ...
      features: [
        ...
        { label: "Tous les modèles", ok: true },
        { label: "Export PDF illimité", ok: true },
        { label: "CV sauvegardés", ok: true },
        { label: "Génération IA", ok: true },
        { label: "Support prioritaire", ok: true },
      ],
      cta: userPlan === "illimite" ? "✅ Plan actuel" : "S'abonner — 5 000F/mois",
      active: userPlan === "illimite", highlight: true, montant: 5000, planKey: "illimite",
    },
  ];
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f0c29, #1e1b4b, #302b63)", display: "flex", flexDirection: "column", alignItems: "center", padding: "40px 20px", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 40 }}>
        ...
        <div style={{ color: "rgba(255,255,255,0.6)", fontSize: 15, marginTop: 8 }}>
          {payMode ? "💳 Payez 500F pour télécharger ce CV" : "Choisissez votre plan"}
        </div>
        <div style={{ display: "inline-block", background: "rgba(99,102,241,0.3)", border: "1px solid rgba(99,102,241,0.5)", borderRadius: 20, padding: "4px 16px", color: "#a5b4fc", fontSize: 12, marginTop: 10 }}>
      ...
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap", justifyContent: "center", maxWidth: 700, width: "100%" }}>
        {plans.map((plan) => (
          <div key={plan.name} style={{ background: plan.highlight ? "linear-gradient(135deg, #4f46e5, #7c3aed)" : "rgba(255,255,255,0.05)", border: plan.highlight ? "2px solid #818cf8" : "1px solid rgba(255,255,255,0.1)", borderRadius: 16, padding: "28px 24px", width: 280, backdropFilter: "blur(12px)", boxShadow: plan.highlight ? "0 20px 60px rgba(79,70,229,0.4)" : "0 4px 20px rgba(0,0,0,0.2)", position: "relative" }}>
            {plan.badge && (
              <div style={{ position: "absolute", top: -12, left: "50%", transform: "translateX(-50%)", background: plan.highlight ? "linear-gradient(135deg, #f59e0b, #d97706)" : "linear-gradient(135deg, #4f46e5, #7c3aed)", borderRadius: 20, padding: "4px 16px", fontSize: 11, fontWeight: 700, color: "#fff", whiteSpace: "nowrap" }}>
                {plan.badge}
              </div>
            )}
            ...
            <div style={{ color: "#fff", fontSize: 30, fontWeight: 800, fontFamily: "'Playfair Display', serif" }}>
              {plan.price}<span style={{ fontSize: 13, fontWeight: 400, opacity: 0.7 }}>{plan.period}</span>
            </div>
            <div style={{ height: 1, background: "rgba(255,255,255,0.15)", margin: "14px 0" }} />
            <div style={{ marginBottom: 20 }}>
              {plan.features.map((f, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, fontSize: 12.5, color: f.ok ? "#fff" : "rgba(255,255,255,0.3)" }}>
                  <span>{f.ok ? "✅" : "❌"}</span>{f.label}
                </div>
              ))}
            </div>
            <button onClick={() => handlePay(plan)} disabled={plan.active || payLoading}
              style={{ width: "100%", padding: "11px", borderRadius: 10, border: "none", fontSize: 12.5, fontWeight: 600, cursor: plan.active ? "default" : "pointer", background: plan.active ? "rgba(255,255,255,0.15)" : plan.highlight ? "#fff" : "rgba(255,255,255,0.15)", color: plan.highlight && !plan.active ? "#4f46e5" : "#fff", fontFamily: "'DM Sans', sans-serif" }}>
              {payLoading ? "⏳ Traitement..." : plan.cta}
            </button>
          </div>
        ))}
      </div>
      <div style={{ marginTop: 40, color: "rgba(255,255,255,0.4)", fontSize: 12, textAlign: "center" }}>
        📱 MTN Mobile Money · Annulation possible · Sans engagement
      </div>
    </div>
  );
}
    ...
    <div style={{ fontFamily: "'Crimson Pro', Georgia, serif", background: t.bg, color: t.text, fontSize: 13, lineHeight: 1.5, minHeight: 800 }}>
      <div style={{ background: t.headerBg, color: t.headerText, padding: "28px 32px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          {data.photo && (
            <img src={data.photo} alt="Photo" style={{ width: 72, height: 72, borderRadius: "50%", objectFit: "cover", border: `3px solid ${t.headerText}40`, flexShrink: 0 }} />
          )}
          <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr" }}>
        <div style={{ background: t.sidebar, padding: "20px 16px", borderRight: `2px solid ${t.accent}20` }}>
          {data.skills && (
            <>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 12, fontWeight: 700, color: t.accent, textTransform: "uppercase", letterSpacing: "2px", borderBottom: `1.5px solid ${t.accent}`, paddingBottom: 3, marginBottom: 8 }}>Compétences</div>
              <div>{data.skills.split(",").map((sk, i) => <span key={i} style={{ display: "inline-block", background: `${t.accent}15`, color: t.accent, borderRadius: 20, padding: "2px 8px", fontSize: 11, margin: "2px 3px 2px 0", fontWeight: 500 }}>{sk.trim()}</span>)}</div>
            </>
          )}
          {data.languages && (
            <>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 12, fontWeight: 700, color: t.accent, textTransform: "uppercase", letterSpacing: "2px", borderBottom: `1.5px solid ${t.accent}`, paddingBottom: 3, marginBottom: 8, marginTop: 16 }}>Langues</div>
              <div>{data.languages.split(",").map((l, i) => <span key={i} style={{ display: "inline-block", background: `${t.accent}15`, color: t.accent, borderRadius: 20, padding: "2px 8px", fontSize: 11, margin: "2px 3px 2px 0", fontWeight: 500 }}>{l.trim()}</span>)}</div>
            </>
          )}
          {data.hobbies && (
            <>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 12, fontWeight: 700, color: t.accent, textTransform: "uppercase", letterSpacing: "2px", borderBottom: `1.5px solid ${t.accent}`, paddingBottom: 3, marginBottom: 8, marginTop: 16 }}>Loisirs</div>
              <div style={{ fontSize: 12 }}>{data.hobbies}</div>
            </>
          )}
          {data.education.some(e => e.school) && (
            <>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 12, fontWeight: 700, color: t.accent, textTransform: "uppercase", letterSpacing: "2px", borderBottom: `1.5px solid ${t.accent}`, paddingBottom: 3, marginBottom: 8, marginTop: 16 }}>Formation</div>
              {data.education.filter(e => e.school).map(e => (
                <div key={e.id} style={{ marginBottom: 8 }}>
                  <div style={{ fontWeight: 600, fontSize: 12 }}>{e.degree}</div>
                  <div style={{ fontSize: 11, fontStyle: "italic", color: t.secondary }}>{e.school}</div>
                  <div style={{ fontSize: 10.5, color: t.secondary }}>{e.period}{e.mention ? ` • ${e.mention}` : ""}</div>
                </div>
              ))}
            </>
          )}
        ...
        <div style={{ padding: "20px 24px" }}>
          {data.summary && (
            <>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 12, fontWeight: 700, color: t.accent, textTransform: "uppercase", letterSpacing: "2px", borderBottom: `1.5px solid ${t.accent}`, paddingBottom: 3, marginBottom: 8 }}>Profil</div>
              <div style={{ fontSize: 12.5, lineHeight: 1.6 }}>{data.summary}</div>
            </>
          )}
          {data.experiences.some(e => e.company) && (
            <>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 12, fontWeight: 700, color: t.accent, textTransform: "uppercase", letterSpacing: "2px", borderBottom: `1.5px solid ${t.accent}`, paddingBottom: 3, marginBottom: 8, marginTop: 16 }}>Expériences</div>
              {data.experiences.filter(e => e.company).map(e => (
                <div key={e.id} style={{ marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <div style={{ fontWeight: 600, fontSize: 13 }}>{e.role}</div>
                    <div style={{ fontSize: 11, color: t.secondary }}>{e.period}</div>
                  </div>
                  <div style={{ fontSize: 11.5, color: t.accent, fontStyle: "italic" }}>{e.company}</div>
                  {e.description && <div style={{ fontSize: 12, marginTop: 2 }}>{e.description}</div>}
                </div>
              ))}
            </>
          )}
...
export default function App() {
  ...
  const [data, setData] = useState(() => {
    try { const s = localStorage.getItem("julius_cv_data"); return s ? JSON.parse(s) : initialData; } catch { return initialData; }
  });
  ...
  const userPlan = isAdmin ? "illimite" : (profile?.plan || "paypercv");
  const cvCount = isAdmin ? 0 : (profile?.cv_count || 0);
  const canDownloadFree = isAdmin || userPlan === "illimite";
  ...
  const handleLogout = async () => {
    ...
    localStorage.setItem("julius_cv_page", "templates");
    setPage("templates");
  };
  ...
  const handleDownloadClick = () => {
    if (canDownloadFree) { printPDF(); }
    else { setPayMode(true); setPage("pricing"); }
  };
  ...
  if (authLoading) return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #0f0c29, #1e1b4b)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontFamily: "'DM Sans', sans-serif" }}>
      ⏳ Chargement...
    </div>
  );
  ...
  if (page === "templates") return (
    <TemplateSelectorPage onSelect={handleTemplateSelect} selectedTemplate={templateKey} />
  );

  if (page === "pricing") return (
    <PricingPage
      onBack={() => { setPayMode(false); setPage("editor"); }}
      onUpgrade={async (plan, isPay) => {
        if (plan === "illimite") setProfile(p => ({ ...p, plan: "illimite" }));
        if (isPay) { await printPDF(); }
        setPayMode(false); setPage("editor");
      }}
      userPlan={userPlan} user={user} payMode={payMode}
    />
  );
        ...
        <div style={{ width: 400, minWidth: 340, display: "flex", flexDirection: "column", background: "#fff", boxShadow: "2px 0 20px rgba(0,0,0,0.08)", zIndex: 10, overflow: "hidden" }}>
          ...
          <div style={{ background: "linear-gradient(135deg, #1e1b4b, #312e81)", padding: "14px 18px", color: "#fff" }}>
            ...
            <div style={{ marginTop: 10, background: "rgba(255,255,255,0.08)", borderRadius: 8, padding: "6px 10px", fontSize: 11, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span>📄 CV créés : {isAdmin ? "∞" : cvCount} | Plan : {isAdmin ? "Admin" : userPlan === "illimite" ? "Illimité" : "Pay-per-CV"}</span>
          ...
          <div style={{ borderTop: "1px solid #e5e7eb", padding: "10px 18px", background: "#fafafa" }}>
            <button onClick={() => setShowPreview(true)}
              style={{ width: "100%", padding: "9px", background: "linear-gradient(135deg, #6366f1, #4f46e5)", color: "#fff", border: "none", borderRadius: 8, fontSize: 12.5, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", marginBottom: 7 }}>
              👁️ Voir mon CV
            </button>
            <button onClick={handleDownloadClick}
              style={{ width: "100%", padding: "9px", background: "linear-gradient(135deg, #1e1b4b, #312e81)", color: "#fff", border: "none", borderRadius: 8, fontSize: 12.5, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.85)", zIndex: 1000, display: "flex", flexDirection: "column" }}>
            <div style={{ background: "linear-gradient(135deg, #1e1b4b, #312e81)", padding: "12px 18px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={handleDownloadClick}
                  style={{ background: "linear-gradient(135deg, #f59e0b, #d97706)", border: "none", borderRadius: 7, padding: "7px 12px", color: "#fff", fontSize: 11.5, fontWeight: 700, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>
                  📄 Télécharger {!canDownloadFree ? "— 500F" : ""}
                </button>
                <button onClick={() => setShowPreview(false)}
                  style={{ background: "rgba(255,255,255,0.15)", border: "none", borderRadius: 7, padding: "7px 12px", color: "#fff", fontSize: 11.5, cursor: "pointer" }}>
                  ✕ Fermer
                </button>
              </div>
            </div>
